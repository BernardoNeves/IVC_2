...
def draw_contours(frame_draw, frame_contours):
    ...
    if len(contours) > 0:
        ...
        _,radius = cv2.minEnclosingCircle(contours_max)
        ...
        cv2.line(frame_draw,(int(center[0]), int(center[1] + 20)), (int(center[0] + 50), int(center[1] + 20)), [0,0,255], 2,8)
        cv2.putText(frame_draw, "Actual", (int(center[0] + 50), int(center[1] + 20)), cv2.FONT_HERSHEY_SIMPLEX,0.5, [0,0,255])
        
        predictedCoords = kfObj.Estimate(center[0],center[1])
        for vertex in box:
            vertex[0] += int(predictedCoords[2])
            vertex[1] += int(predictedCoords[3])
            
        cv2.drawContours(frame_draw, [box], 0, (255, 0, 0), 2)
        
        cv2.line(frame_draw, (int(predictedCoords[0]) + 16, int(predictedCoords[1]) - 15), (int(predictedCoords[0]) + 50, int(predictedCoords[1]) - 30), [255, 0, 0], 2, 80)
        cv2.putText(frame_draw, "Predicted", (int(predictedCoords[0] + 50), int(predictedCoords[1] - 30)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [255, 0, 0])
